...
| containers | List of containers to deploy in the container group | `list(object)` | n/a | yes |
| os_type | The operating system type for the container group (Linux or Windows) | `string` | `"Linux"` | no |
| restart_policy | The restart policy for the container group (Always, OnFailure, Never) | `string` | `"Always"` | no |
| enable_public_ip | Enable public IP address for the container group | `bool` | `true` | no |
| ip_address_type | The IP address type for the container group (Public, Private, None) | `string` | `"Public"` | no |
| dns_name_label | The DNS name label for the container group | `string` | `null` | no |
| dns_name_label_reuse_policy | The DNS name label reuse policy | `string` | `"Unsecure"` | no |
| exposed_ports | List of ports to expose on the container group | `list(object)` | `[]` | no |
| enable_vnet_integration | Enable VNet integration for the container group | `bool` | `false` | no |
| subnet_ids | List of subnet IDs for VNet integration | `list(string)` | `[]` | no |
| volumes | List of volumes to mount in the container group | `list(object)` | `[]` | no |
| image_registry_credentials | List of image registry credentials | `list(object)` | `[]` | no |
| identity_type | The type of managed identity for the container group | `string` | `null` | no |
| identity_ids | List of user assigned identity IDs | `list(string)` | `[]` | no |
| enable_diagnostic_settings | Enable diagnostic settings for the container group | `bool` | `false` | no |
| log_analytics_workspace_id | Log Analytics workspace ID for diagnostic settings | `string` | `null` | no |
| diagnostic_logs | List of log categories to enable for diagnostic settings | `list(string)` | `["ContainerInstanceLog"]` | no |
| diagnostic_metrics | List of metric categories to enable for diagnostic settings | `list(string)` | `["AllMetrics"]` | no |
| priority | The priority of the container group (Regular, Spot) | `string` | `"Regular"` | no |
| zones | List of availability zones for the container group | `list(string)` | `[]` | no |
...
| containers | Information about the containers in the Container Group |
| volumes | Information about the volumes in the Container Group |
| identity | The managed identity of the Container Group |
| principal_id | The principal ID of the Container Group's managed identity |
| runtime_configuration | Runtime configuration of the Container Group |
| network_configuration | Network configuration of the Container Group |
| security_configuration | Security configuration of the Container Group |
| monitoring_configuration | Monitoring configuration of the Container Group |
| container_group_summary | Summary of the Container Group deployment |
